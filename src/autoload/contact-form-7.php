...
function __cf7_tag_content_as_label($tag = null){
    ...
    return __remove_whitespaces($tag->content);
}
 ...
 * @return bool
 */
function __cf7_tag_is_false($name = '', $tag = null){
    if(!$tag instanceof \WPCF7_FormTag){
        return false;
    }
	...
	return __is_false($tag->get_option($name, '', true));
}

/**
 * @return bool
 */
function __cf7_tag_is_true($name = '', $tag = null){
    if(!$tag instanceof \WPCF7_FormTag){
        return false;
    }
	if(!$tag->has_option($name)){
	...
	return __is_true($tag->get_option($name, '', true));
}

/**
 * @return bool
 */
function __cf7_tag_label_equals($tag = null){
    if(!$tag instanceof \WPCF7_FormTag){
        return false;
    }
	$content_label = __cf7_tag_content_as_label($tag);
	$placeholder = __cf7_tag_placeholder($tag);
	if(!$content_label and !$placeholder){
		return false;
	}
	return ($content_label === $placeholder);
}
	...
	if('select' === $tag->basetype){
		if($tag->has_option('include_blank') or empty($tag->values)){
			if(version_compare(WPCF7_VERSION, '5.7', '>=')){
				return __('&#8212;Please choose an option&#8212;', 'contact-form-7'); // Drop-down menu: Uses more friendly label text. https://contactform7.com/2022/12/10/contact-form-7-57/
			} else {
				return '---';
			}
