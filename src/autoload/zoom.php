...
function __zoom_access_token(){
    if(__isset_cache('zoom_access_token')){
        return __get_cache('zoom_access_token');
    }
...
function __zoom_app_credentials($app_credentials = []){
    if(__isset_cache('zoom_app_credentials')){
        return __get_cache('zoom_app_credentials');
    }
    ...
    if($missing){
		$message = sprintf(__('Missing parameter(s): %s'), __implode_and($missing)) . '.';
...
function __zoom_oauth_token(){
    $transient = __prefix('zoom_oauth_token');
    ...
    $expiration = 58 * MINUTE_IN_SECONDS; // The token’s time to live is 1 hour. https://developers.zoom.us/docs/internal-apps/s2s-oauth/
