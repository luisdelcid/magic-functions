...
function __add_external_rule($regex = '', $query = '', $plugin_file = ''){
    ...
    __set_cache('external_rules', $external_rules);
    __one('admin_init', '__maybe_add_external_rules_notice');
    __one('generate_rewrite_rules', '__maybe_add_external_rules');
}
...
function __maybe_add_external_rules_notice(){
    ...
    $message = sprintf(__('You should update your %s file now.'), '<code>.htaccess</code>');
    $message .= ' ';
    $message .= sprintf('<a href="%s">%s</a>', esc_url(admin_url('options-permalink.php')), __('Flush permalinks')) . '.';
