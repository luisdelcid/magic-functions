...
function __cf7_contact_form($contact_form = null){
	...
	$post_id = __cf7_hash_exists($contact_form);
	if($post_id){
...
function __cf7_contact_form_by($field = '', $value = null){
    if('hash' === $field and version_compare(WPCF7_VERSION, '5.8', '<')){
		return null;
	}
	if('id' === $field){
	...
	switch($field){
		case 'hash':
			$contact_form = wpcf7_get_contact_form_by_hash($value); // Hash-based contact form identification: https://contactform7.com/2023/08/06/contact-form-7-58/#hash-based-contact-form-identification
			break;
		case 'id':
...
function __cf7_fake_mail($contact_form = null, $submission = null){
	...
	if(__cf7_skip_mail($contact_form) or __cf7_send_mail($contact_form)){
