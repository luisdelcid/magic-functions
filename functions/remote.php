...
function __is_cloudflare(){
	return !empty($_SERVER['CF-ray']); // TODO: check for Cloudflare Enterprise
}

/**
 * @return bool|WP_Error
 */
function __is_remote_error($error = []){
	...
	$count = count($error);
	if((4 === $count and !array_key_exists('additional_errors', $error)) or 3 !== $count){
		return false;
	}
...
function __remote_request($method = '', $url = '', $args = []){
	...
	$is_json = __remote_request_is_json($args);
	...
	if($is_json){
		...
		$error = __is_remote_error($json_params);
		if(is_wp_error($error)){
...
function __remote_request_is_json($args = []){
	if(!__is_wp_http_request($args, true)){
		return __error(__('Invalid request method.'));
	}
...
function __sanitize_timeout($timeout = 0){
	...
	if(__is_cloudflare()){
		if(0 === $timeout or $timeout > 98){
			$timeout = 98; // If the max_execution_time is set to greater than 98 seconds, reduce it a bit to prevent edge-case timeouts that may happen before the page is fully loaded. https://developers.cloudflare.com/support/troubleshooting/cloudflare-errors/troubleshooting-cloudflare-5xx-errors/#error-524-a-timeout-occurred
		}
	}
	return $timeout;
}
